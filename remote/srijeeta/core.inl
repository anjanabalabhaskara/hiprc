...
int parse_trace_input(int* index)
{
    ...
    if (CREATE_SUB_KERNEL)
        task_info->partition_count=4;//########################hard codedrigth now########
...
{   
    ...
    {   
        ...
        printf("******Kernel Stat: %d p-d: %d-%d size: %u / %u freq:%u totla-time:%llu nd-time: %llu relative start-end:%llu-%llu *******\n",kl->task->traceID,kl->platform_pos,kl->device_pos,kl->size,kl->task->task_size,kl->frequency,timing,kl->kex.nd_time ,kl->kex.rel_start_time,kl->kex.rel_end_time);
...
int global_recovery(std::vector<std::pair<int, int>> violated_tasks)
{
    for(int i=0;i<violated_tasks.size();i++)
    {
        ...
        if(iter != taskMap.end()) //if found
        {
            std::vector<KernelLaunchInfo*> kl_vector = iter->second; 
            //update kernelinfos for all partitions of task



            kl_vector[0]->task->majorViolationPerHP=0;
...
int find_global_recovery_mode(std::pair<int, int> t_violation, KernelLaunchInfo* next_klinfo)
{
    
 /*   if(t_violation.first==2) //minor violation
    {
        //find the local deadline for next task
        unsigned long long int local_dl=next_klinfo->WCET+BUFFER_TIME+get_current_time()

        if(ready_buffer[PLATFORM_GPU][0].size()>0)
    }*/
    return 0;
}
